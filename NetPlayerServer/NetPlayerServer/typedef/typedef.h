...
struct Pt_Matrix : public _pt_matrix
{
public:
    Pt_Matrix() {};
    Pt_Matrix( const _pt_matrix& ptmat)
	{
...
struct pt_point
{
	...
	pt_point operator*(_pt_matrix mat)
	{
		pt_point point;
...
#define PT_STRING_DEFAULT_LENGTH 32
	...
	friend pt_string operator+(const char str[],pt_string &ptstr)
	{
		pt_char shl=0;
		pt_string  ptstring;
		ptstring.free();
		pt_uint length=strlen(str)+ptstr.stringlength();
		while ((pt_uint)(1<<++shl)<=length);
	...
	pt_string operator+(pt_string& ptstr)
	{
		...
		ptstring.buffer=(pt_char *)new pt_char[ptstring.bufferlen];
		ptstring.buffer[0]='\0';
		PT_STRCAT(ptstring.buffer,buffer);
		PT_STRCAT(ptstring.buffer,ptstr.buffer);
	...
	pt_string &replace(pt_string sourceString,pt_string replaceString)
	{
		if (sourceString.stringlength()==0)
		{
			return *this;   
		}
		  pt_string str;
		  int i;
		  for ( i=0;i<this->stringlength()-sourceString.stringlength();i++)
		  {
			  if (memcmp(buffer+i,sourceString.buffer,sourceString.stringlength())==0)
			  {
				  str+=replaceString;
			  }
			  else
	...
	pt_string& trim()
	{
		if (buffer)
		{
			...
			while(PT_TRUE)
			{
				if (buffer[loft]=='\0'||loft>=bufferlen)
				{
					break;
				}
				buffer[i++]=buffer[loft++];
			}
			buffer[i]='\0';
			i=strlen(buffer);
			if (i)
	...
	pt_bool IsStringFloat()
	{
		return IsStringNumeric()&&m_IsNumericFloat;
	}
	...
	pt_bool IsStringNumeric()
	{
		m_IsNumericFloat=PT_FALSE;
		pt_int Dot=PT_FALSE;
		pt_string pString=*this;
		if (pString.stringlength()==0)
		{
			return PT_FALSE;
		}
		pt_uint CurrentCharIndex;

		if (!IsCharNumeric(pString.buffer[0])&&!(pString.buffer[0]==('-')))
		{
			if(pString.buffer[0]!='.')
				return PT_FALSE;
		}
		for (CurrentCharIndex=1;
			...
			CurrentCharIndex++)
		{
			if (!IsCharNumeric(pString.buffer[CurrentCharIndex]))
			{
				if (pString.buffer[CurrentCharIndex]=='e'&&(pString.buffer[CurrentCharIndex+1]=='+'||pString.buffer[CurrentCharIndex+1]=='-'))
				{
					CurrentCharIndex++;
					continue;
				}

				if (pString.buffer[CurrentCharIndex]=='.')
				{
					if (Dot)
					{
						return PT_FALSE;
...
private:
	...
	pt_int IsCharNumeric(pt_char chr)
	{
		if (chr>=('0')&&chr<=('9'))
		{
			return PT_TRUE;
